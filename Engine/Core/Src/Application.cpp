...
void CApplication::InitApplication( uint width, uint height )
{
	...
	m_pRenderPipeline->mainCamera.transform.position = Vec3(0.0f, -300.0f, 80.0f);


	gVars->pResourceManager->ParseResources( "" );

	
 	pCharacterSkel = "ThirdPersonRun.skel";
 ...
 //	pHouse->SetLocalTransform(Transform(Vec3(0.0f, 800.0f, 0.0f), Quat(), Vec3(1.0f)));
	...
	//pThird->SetShaderProgram(0, "skinning.program");
		...
		pCharacterRenderNode = new CSkinnedMeshRenderNode;

		pCharacterRenderNode->SetLocalTransform(Transform(Vec3(0, 0, 0.0f), Quat()));

		pCharacterRenderNode->AttachTo(&rootNode);
		pCharacterRenderNode->SetMesh("SK_Mannequin.msh");
		pCharacterRenderNode->SetShaderProgram(0, "skinning.program");
...
// 	}
	...
	gVars->pResourceManager->UpdateResourceLoading();
		...
		if (gVars->pSimulation)
	{
		gVars->pSimulation->Init();
	}
...
//	}
	...
	if (!bMovingCamera) SetCursor(LoadCursor(NULL, IDC_ARROW));
    else                SetCursor(NULL);
	
	if (m_bMovingCamera != bMovingCamera)
	{
		m_bMovingCamera = bMovingCamera;
	...
	if (m_bMovingCamera)
	{
		...
		// m_prevMousePos = mousePos;
		gVars->pOperatingSystem->SetMousePos(m_prevMousePos);
	}
	m_pRenderPipeline->mainCamera.transform.rotation = Quat::CreateRotZ(Radian(m_camYaw)) *Quat::CreateRotX(Radian(m_camPitch));
...
// 		pNode->AddImpulse(m_pRenderPipeline->mainCamera.transform.rotation * Vec3::Forward * 100.0f);
	...
	if ( gVars->pSimulation )
	{
		gVars->pSimulation->Update(frameTime);
	}
