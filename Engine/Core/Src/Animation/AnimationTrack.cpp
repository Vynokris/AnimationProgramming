...
void	CAnimationTrack::GetKeyFrame2( size_t index, Transform& keyFrame ) const
{
...
void	CAnimationTrack::GetKeyFrame(size_t index, float& w, float& x, float& y, float& z, Vec3& position) const
{
	const Transform& keyframe = ((Transform const*)m_keyFrames)[index];
...
void	CAnimationTrack::Sample( float animTime, Transform& fromKeyFrame, Transform& toKeyFrame, float& weight ) const
{
	...
	GetKeyFrame2( fromKeyIndex, fromKeyFrame );
	GetKeyFrame2( toKeyIndex, toKeyFrame );
