...
{
    if (ImGui::Begin("Stats", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
    {
    ...
    if (ImGui::Begin("Animations", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize))
    {
        Animator& animator = skeleton.GetAnimator();
        ...
        ImGui::Unindent(4);
        ImGui::BulletText("Current animation:");
        ImGui::Indent(4);
        ImGui::Indent(16);
        ...
        if (ImGui::BeginCombo("##curAnimInput", (isAnimating ? curAnim->nameNoExtension.c_str() : "none")))
        {
            ...
            for (auto& [name, anim] : animations)
            {
                ...
                if (ImGui::Selectable(anim->nameNoExtension.c_str(), isSelected) && !animator.IsTransitioning()) {
        ...
        {
            ImGui::Unindent(4);
            ImGui::BulletText("Animation transition:");
            ImGui::Indent(4);
            ImGui::Indent(16);
            if (!animator.IsTransitioning())
            {
                static float transitionDuration = 2.f;
                if (ImGui::Button("Start") && animator.IsAnimating() && !animator.IsTransitioning())
                {
            ...
            else
            {
                ...
                ImGui::Text("In transition (%d%%)", roundInt(animator.GetTransitionCompletion() * 100));
            }
            ImGui::Unindent(16);
            ImGui::NewLine();
        }
        ...
        ImGui::Unindent(4);
        ImGui::BulletText("Animation parameters:");
        ImGui::Indent(4);
        ImGui::Indent(19);
        for (auto& [name, anim] : animations)
        {
