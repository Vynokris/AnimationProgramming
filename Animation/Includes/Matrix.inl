...
inline float* Matrix<R, C>::floatPtr()
{
    return &m[0][0];
}
...
inline Matrix<(R>_R ? R : _R), (C>_C ? C : _C)> Matrix<R, C>::operator*(const Matrix<_R, _C>& matrix) const
{
...
template<int R, int C>
inline Matrix<R, C> Matrix<R, C>::transpose() const
{
    Matrix<C, R> result;
    for (int i = 0; i < R; i++)
        for (int j = 0; j < C; j++)
...
template<int R, int C>
inline Quaternion Matrix<R, C>::toQuaternion()
{
    assert(R == C && (R == 3 || R == 4));
...
template<int R, int C>
inline std::string Matrix<R, C>::printStr(const bool& showSize) const
{
    std::string output = "";
